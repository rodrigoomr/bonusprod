...
export async function getConfig() {
  ...
  if (error) { console.error('getConfig:', error); return { pointValue: 0.5, minGoalPercent: 100, finalSectorId: 's1', workingDays: 0, pointsPerDay: 90, monthlyGoal: 0 }; }
  return {
    pointValue: Number(data.point_value),
    minGoalPercent: data.min_goal_percent,
    finalSectorId: data.final_sector_id || 's1',
    workingDays: data.working_days || 0,
    pointsPerDay: Number(data.points_per_day || 90),
...
export async function saveConfig(config) {
  const row = {
    ...
    min_goal_percent: config.minGoalPercent,
    final_sector_id: config.finalSectorId,
    working_days: config.workingDays || 0,
    points_per_day: config.pointsPerDay || 90,
    monthly_goal: config.monthlyGoal || 0
  }
...
export async function deleteHistory(id) {
  ...
  if (error) console.error('deleteHistory:', error)
}
