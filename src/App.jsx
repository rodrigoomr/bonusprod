...
// ── Stats: meta global em pontos (dias úteis × pts/dia) ──
function computeEmployeeStats(users, sectors, production, config, goalOverride, monthKeyOverride, minGoalOverride) {
  const mk = monthKeyOverride || getMonthKey();
  const today = getToday();
  const finalId = config.finalSectorId || 's1';
  const autoWD = getCurrentMonthWorkingDays();
  const wd = config.workingDays && config.workingDays > 0 ? config.workingDays : autoWD;
  const ppd = config.pointsPerDay || 90;
  const globalGoal = goalOverride || (config.monthlyGoal && config.monthlyGoal > 0 ? config.monthlyGoal : wd * ppd) || (wd * ppd);
  const minPct = minGoalOverride !== undefined ? minGoalOverride : (config.minGoalPercent || 100);
  return users.filter(u => u.role === "employee").map(u => {
    ...
    // Filtrar apenas produção do mês correto
...
export default function App() {
  ...
  const canLaunch = isAdmin; // only admin can launch/edit/delete production
  const userSectorNames = sectors.filter(s => getUserSectorIds(currentUser).includes(s.id)).map(s => s.name).join(", ") || "Colaborador";
  const roleLabel = isAdmin ? "Administrador" : isSupervisor ? "Supervisor" : userSectorNames;
  const staffNavItems = [
    {id:"dashboard",label:"Dashboard",icon:Icons.dashboard,section:"Principal"},
    {id:"production",label:"Produção",icon:Icons.production,section:"Principal"},
    {id:"ranking",label:"Ranking",icon:Icons.ranking,section:"Principal"},
    {id:"reports",label:"Relatórios",icon:Icons.reports,section:"Principal"},
    {id:"users",label:"Usuários",icon:Icons.users,section:"Gestão"},
    {id:"sectors",label:"Setores",icon:Icons.sectors,section:"Gestão"},
    {id:"history",label:"Histórico",icon:Icons.history,section:"Gestão"},
    {id:"settings",label:"Configurações",icon:Icons.settings,section:"Gestão"},
    {id:"logs",label:"Logs",icon:Icons.logs,section:"Gestão"},
  ];
  ...
  return (
    <div className="app">
      ...
      <aside className={`sidebar ${sidebarOpen?"open":""}`}>
        ...
        <div className="sidebar-footer"><div className="user-badge"><div className="user-avatar">{currentUser.name.charAt(0)}</div><div className="user-info"><div className="user-name">{currentUser.name}</div><div className="user-role">{roleLabel}</div></div><button className="btn-logout" onClick={()=>setShowChangePwd(true)} title="Alterar Senha" style={{marginRight:4}}><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0110 0v4"/></svg></button><button className="btn-logout" onClick={()=>setCurrentUser(null)} title="Sair">{Icons.logout}</button></div><div style={{textAlign:"center",fontSize:10,color:"var(--text-muted)",marginTop:10,fontFamily:"'Space Mono',monospace"}}>BonusProd {APP_VERSION}</div></div>
      </aside>
      <main className="main">
        ...
        <div className="content fade-in" key={page}>
          {page==="dashboard"&&(isStaff?<AdminDashboard users={users} sectors={sectors} production={production} config={config}/>:<EmployeeDashboard user={currentUser} sectors={sectors} production={production} config={config}/>)}
          {page==="production"&&isStaff&&<ProductionPage users={users} sectors={sectors} production={production} setProduction={setProduction} addLog={addLogEntry} canLaunch={canLaunch}/>}
          {page==="ranking"&&<RankingPage users={users} sectors={sectors} production={production} config={config} isAdmin={isStaff}/>}
          {page==="myproduction"&&!isStaff&&<MyProductionPage currentUser={currentUser} sectors={sectors} production={production}/>}
          {page==="reports"&&isStaff&&<ReportsPage users={users} sectors={sectors} production={production} config={config}/>}
          {page==="users"&&isStaff&&<UsersPage users={users} setUsers={setUsers} sectors={sectors} addLog={addLogEntry} isAdmin={isAdmin}/>}
          {page==="sectors"&&isStaff&&<SectorsPage sectors={sectors} setSectors={setSectors} addLog={addLogEntry} isAdmin={isAdmin}/>}
          ...
          {page==="settings"&&isStaff&&<SettingsPage config={config} setConfig={setConfig} sectors={sectors} addLog={addLogEntry} isAdmin={isAdmin}/>}
          {page==="logs"&&isStaff&&<LogsPage logs={logs}/>}
        </div>
      </main>
...
function AdminDashboard({ users, sectors, production, config }) {
  ...
  const ppd = config.pointsPerDay || 90;
  const globalGoal = config.monthlyGoal && config.monthlyGoal > 0 ? config.monthlyGoal : workingDays * ppd;
  ...
  return (
    <div>
      ...
      {/* Barra de dias úteis + meta global */}
      <div className="card" style={{marginBottom:20,padding:"14px 18px"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:16}}>
          ...
          <div style={{borderLeft:"1px solid #2A2E3F",paddingLeft:24,display:"flex",gap:24,alignItems:"center",flexWrap:"wrap"}}>
            <div>
              <span style={{fontSize:11,color:"var(--text-muted)",textTransform:"uppercase",letterSpacing:1,fontWeight:600}}>Meta Mensal Global</span>
              <div className="mono" style={{fontSize:24,fontWeight:800,color:"var(--accent)"}}>{globalGoal} <span style={{fontSize:12,fontWeight:400,color:"var(--text-muted)"}}>pts</span></div>
              <div style={{fontSize:11,color:"var(--text-muted)",marginTop:2}}>{ppd} pts/dia × {workingDays} dias úteis{config.monthlyGoal>0?" (manual)":""}</div>
            </div>
          </div>
        </div>
      </div>
...
function ProductionPage({ users, sectors, production, setProduction, addLog, canLaunch }) {
  ...
  return (
    <div>
      {canLaunch ? (
        ...
        <div className="card" style={{marginBottom:24,padding:"14px 18px",display:"flex",alignItems:"center",gap:12}}>
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="var(--warning)" strokeWidth="2"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
          <span style={{fontSize:14,color:"var(--text-muted)"}}>Supervisores têm acesso somente de visualização. Para lançar produção, utilize uma conta de Administrador.</span>
        </div>
      )}
      ...
      <div className="card"><div className="card-header"><span className="card-title">Lançamentos — {formatDate(filterFrom)} {filterFrom!==filterTo?` até ${formatDate(filterTo)}`:""}</span>
        ...
        <div className="table-wrapper"><table><thead><tr><th>Data</th><th>Colaborador</th><th>Setor</th><th>Qtd</th><th>Pontos</th>{canLaunch&&<th>Ações</th>}</tr></thead><tbody>
          {filteredEntries.length===0?<tr><td colSpan={canLaunch?6:5} style={{textAlign:"center",color:"var(--text-muted)",padding:30}}>Nenhum lançamento no período</td></tr>
          :filteredEntries.map(e=>{const u=users.find(x=>x.id===e.userId);const s=sectors.find(x=>x.id===e.sectorId);return(
            <tr key={e.id}><td className="mono" style={{fontSize:12}}>{formatDate(e.date)}</td><td style={{fontWeight:600}}>{u?.name||"—"}</td><td>{s?.name||"—"}</td><td className="mono">{e.quantity}</td><td className="mono accent">{e.quantity*(s?.pointsPerUnit||0)}</td>{canLaunch&&<td><div style={{display:"flex",gap:6}}><button className="btn btn-secondary btn-sm" onClick={()=>setEditEntry(e)}>Editar</button><button className="btn btn-danger btn-sm" onClick={()=>handleDelete(e.id)}>Excluir</button></div></td>}</tr>);})}</tbody></table></div></div>
      {canLaunch&&importModal&&<ExcelImportModal users={users} sectors={sectors} setProduction={setProduction} addLog={addLog} onClose={()=>setImportModal(false)}/>}
      {canLaunch&&editEntry&&<EditProductionModal entry={editEntry} users={users} sectors={sectors} onSave={handleEditSave} onClose={()=>setEditEntry(null)}/>}
    </div>);
}
  ...
  const handleFile = (e)=>{
    ...
    reader.onload = (evt)=>{
      try {
        ...
        setRawJson(json);
        processRows(json, importDate);
      } catch(err) { setErrors(["Erro ao ler arquivo: "+err.message]); }
    };
    reader.readAsArrayBuffer(file);
  };

  // Reprocessar quando a data muda (se já tem arquivo carregado)
  useEffect(()=>{ if(rawJson) processRows(rawJson, importDate); }, [importDate]);

  // Mapeamento de códigos do Previncêndio para nomes de setores no BonusProd
  const PREVINCENDIO_MAP = {
    'acab':'Acabamento', 'poabc':'Carga ABC', 'agua':'Carga Agua', 'pobc':'Carga BC',
    'co':'Carga CO2', 'decap':'Decapagem', 'desm_abc':'Desmonte ABC', 'desm_agua':'Desmonte Agua',
    'desm_bc':'Desmonte BC', 'desm_co2':'Desmonte Co2', 'estanq':'Estanqueidade',
    'limp':'Limpeza de Valvula', 'pint':'Pintura'
  };

  const processRows = (json, dateOverride)=>{
    const errs = []; const parsed = [];
    const employees = users.filter(u=>u.role==="employee");

    // Auto-detectar formato Previncêndio (colunas RESPONSÁVEL + SETOR + MÊS + QUANTIDADE)
    const firstRow = json[0] || {};
    const keys = Object.keys(firstRow);
    const isPrevincendio = keys.some(k=>k.toUpperCase()==='RESPONSÁVEL') && keys.some(k=>k.toUpperCase()==='MÊS');

    json.forEach((row,i)=>{
      const lineNum = i+2;
      let nameVal, sectorVal, qtyVal, dateVal;

      if(isPrevincendio){
        // Formato Previncêndio — usa a data informada pelo usuário
        nameVal = String(row['RESPONSÁVEL']||row['Responsável']||row['responsável']||"").trim();
        const sectorCode = String(row['SETOR']||row['Setor']||row['setor']||"").trim().toLowerCase();
        sectorVal = PREVINCENDIO_MAP[sectorCode] || sectorCode;
        qtyVal = parseInt(row['QUANTIDADE']||row['Quantidade']||row['quantidade']||0);
        dateVal = dateOverride || getToday();
      } else {
        // Formato padrão BonusProd
        nameVal = String(row['colaborador']||row['Colaborador']||row['COLABORADOR']||row['nome']||row['Nome']||row['NOME']||"").trim();
        sectorVal = String(row['setor']||row['Setor']||row['SETOR']||"").trim();
        qtyVal = parseInt(row['quantidade']||row['Quantidade']||row['QUANTIDADE']||row['qtd']||row['Qtd']||row['QTD']||0);
        dateVal = row['data']||row['Data']||row['DATA']||"";
      }
      ...
      // Match colaborador (case insensitive, parcial)
      const emp = employees.find(u=>u.name.toLowerCase()===nameVal.toLowerCase()) || employees.find(u=>u.name.toLowerCase().includes(nameVal.toLowerCase())) || employees.find(u=>nameVal.toLowerCase().includes(u.name.toLowerCase()));
      ...
      if(sectorVal){
        ...
        if(!sec){
          // Tentar pelo código Previncêndio se o nome direto não encontrar
          const mapped = PREVINCENDIO_MAP[sectorVal.toLowerCase()];
          if(mapped) sec = sectors.find(s=>s.name.toLowerCase()===mapped.toLowerCase());
      ...
      // Date handling
      let dateStr = "";
      if(dateVal instanceof Date){dateStr = dateToStr(dateVal);}
      ...
      {done ? (
        ...
        <div style={{marginBottom:20}}>
          <div style={{padding:16,background:"var(--bg-input)",borderRadius:8,marginBottom:16}}>
            <div style={{fontWeight:600,fontSize:14,marginBottom:8}}>Formatos aceitos:</div>
            <div style={{fontSize:12,color:"var(--text-secondary)",lineHeight:1.8}}>
              <span style={{fontWeight:700,color:"var(--accent)"}}>✓ Previncêndio (automático)</span> — planilha exportada do sistema com colunas RESPONSÁVEL, SETOR, MÊS, QUANTIDADE. Os códigos dos setores são convertidos automaticamente.<br/>
              <span style={{fontWeight:700,color:"var(--blue)"}}>✓ Formato BonusProd</span> — colunas Colaborador, Setor, Quantidade, Data (DD/MM/AAAA)
            </div>
          </div>
          <div style={{display:"flex",gap:16,alignItems:"end",marginBottom:16,flexWrap:"wrap"}}>
            <div className="form-group" style={{margin:0,flex:"0 0 200px"}}>
              <label className="form-label" style={{fontWeight:700,color:"var(--accent)"}}>Data da Produção</label>
              <input className="form-input" type="date" value={importDate} onChange={e=>{setImportDate(e.target.value);}}/>
              <div style={{fontSize:11,color:"var(--text-muted)",marginTop:4}}>Todos os lançamentos usarão esta data</div>
            </div>
            <div className="form-group" style={{margin:0,flex:1}}>
              <label className="form-label">Selecionar Planilha</label>
...
function UsersPage({ users, setUsers, sectors, addLog, isAdmin }) {
  ...
  const roleBadge = (role) => role==="admin"?"yellow":role==="supervisor"?"purple":"blue";
  const roleLabel = (role) => role==="admin"?"Admin":role==="supervisor"?"Supervisor":"Colaborador";
  return (<div><div className="card"><div className="card-header"><span className="card-title">Usuários ({users.length})</span>
    {isAdmin && <div style={{display:"flex",gap:8}}><button className="btn btn-secondary btn-sm" onClick={()=>setBatchModal(true)}>Editar Metas em Lote</button><button className="btn btn-primary btn-sm" onClick={()=>setModal("add")}>+ Novo</button></div>}</div>
    <div className="table-wrapper"><table><thead><tr><th>Nome</th><th>Login</th><th>Perfil</th><th>Setores</th><th>Meta (pts)</th>{isAdmin&&<th>Ações</th>}</tr></thead><tbody>
      {users.map(u=>{const ids=getUserSectorIds(u);const names=sectors.filter(s=>ids.includes(s.id)).map(s=>s.name);return(
        <tr key={u.id}><td style={{fontWeight:600}}>{u.name}</td><td className="mono" style={{color:"var(--text-muted)"}}>{u.login}</td>
          ...
          <td className="mono">{u.role==="employee"?u.pointsGoal||0:"—"}</td>
          {isAdmin&&<td><div style={{display:"flex",gap:8}}><button className="btn btn-secondary btn-sm" onClick={()=>setModal(u)}>Editar</button>{u.id!=="u0"&&<button className="btn btn-danger btn-sm" onClick={()=>handleDelete(u.id)}>Excluir</button>}</div></td>}
          </tr>);})}
    </tbody></table></div></div>
    {isAdmin&&modal&&<Modal title={modal==="add"?"Novo Usuário":"Editar Usuário"} onClose={()=>setModal(null)}><UserForm user={modal==="add"?null:modal} sectors={sectors} onSave={handleSave} onClose={()=>setModal(null)}/></Modal>}
    {isAdmin&&batchModal&&<BatchGoalModal users={users} setUsers={setUsers} addLog={addLog} onClose={()=>setBatchModal(false)}/>}
  </div>);
}
...
function UserForm({ user, sectors, onSave, onClose }) {
  ...
  return (<>
    ...
    <div className="grid-2">
      <div className="form-group"><label className="form-label">Perfil</label><select className="form-select" value={form.role} onChange={e=>setForm({...form,role:e.target.value})}>
...
function SectorsPage({ sectors, setSectors, addLog, isAdmin }) {
  const [modal,setModal] = useState(null);
  const [batchModal,setBatchModal] = useState(false);
  ...
  return (<div>
    {!isAdmin&&<div className="card" style={{marginBottom:16,padding:"10px 16px",display:"flex",alignItems:"center",gap:10}}><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--warning)" strokeWidth="2"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg><span style={{fontSize:13,color:"var(--text-muted)"}}>Visualização apenas. Supervisores não podem editar setores.</span></div>}
    <div className="card"><div className="card-header"><span className="card-title">Setores ({sectors.length})</span>
    {isAdmin&&<div style={{display:"flex",gap:8}}><button className="btn btn-secondary btn-sm" onClick={()=>setBatchModal(true)}>Editar Pontos em Lote</button><button className="btn btn-primary btn-sm" onClick={()=>setModal("add")}>+ Novo</button></div>}</div>
    <div className="table-wrapper"><table><thead><tr><th>Setor</th><th>Pontos/Un</th><th>Tipo</th>{isAdmin&&<th>Ações</th>}</tr></thead><tbody>
      {sectors.map(s=><tr key={s.id}><td style={{fontWeight:600}}>{s.name}</td><td className="mono accent">{s.pointsPerUnit}</td><td>{s.isFinal?<span className="badge green">★ Setor Final</span>:<span className="badge blue">Intermediário</span>}</td>{isAdmin&&<td><div style={{display:"flex",gap:8}}><button className="btn btn-secondary btn-sm" onClick={()=>setModal(s)}>Editar</button><button className="btn btn-danger btn-sm" onClick={()=>handleDelete(s.id)}>Excluir</button></div></td>}</tr>)}
    </tbody></table></div></div>
    {isAdmin&&modal&&<Modal title={modal==="add"?"Novo Setor":"Editar Setor"} onClose={()=>setModal(null)}><SectorForm sector={modal==="add"?null:modal} onSave={handleSave} onClose={()=>setModal(null)}/></Modal>}
    {isAdmin&&batchModal&&<BatchSectorPointsModal sectors={sectors} setSectors={setSectors} addLog={addLog} onClose={()=>setBatchModal(false)}/>}
  </div>);
}
...
function HistoryPage({ history, setHistory, users, sectors, production, config, addLog, isAdmin, currentUser }) {
  ...
  const [closeMonthKey, setCloseMonthKey] = useState(mk);

  // Calcular meta sugerida para o mês escolhido
  const getSuggestedGoal = (monthKey) => {
    const [y,m] = monthKey.split("-").map(Number);
    let wd = 0;
    const daysInMonth = new Date(y,m,0).getDate();
    for(let d=1;d<=daysInMonth;d++){const dow=new Date(y,m-1,d).getDay();if(dow!==0&&dow!==6)wd++;}
    const ppd = config.pointsPerDay||90;
    return wd * ppd;
  };
  const [closeGoal, setCloseGoal] = useState(()=>getSuggestedGoal(mk));
  const [closeMinPct, setCloseMinPct] = useState(config.minGoalPercent || 100);

  // Atualizar meta sugerida quando muda o mês
  const handleMonthChange = (key) => {
    setCloseMonthKey(key);
    setCloseGoal(getSuggestedGoal(key));
  };
  ...
  const handleClose = async () => {
    // USA TODA A PRODUÇÃO — computeEmployeeStats filtra pelo monthKeyOverride internamente
    const stats = computeEmployeeStats(users, sectors, production, config, closeGoal, closeMonthKey, closeMinPct);
    const entry = {
      id: genId(), monthKey: closeMonthKey, pointValue: config.pointValue,
      monthlyGoal: closeGoal, minGoalPercent: closeMinPct,
      employees: stats.map(e => ({ userId:e.id, name:e.name, sectors:e.sectorNames, points:e.points, goal:closeGoal, bonusValue:e.bonusValue, bonus:e.bonus, pct:e.pct, metGoal:e.metGoal }))
    };
    await db.addHistory(entry);
    setHistory(prev => [{ ...entry, closedAt: new Date().toISOString() }, ...prev]);
    addLog("Fechamento", `Mês ${monthLabel(closeMonthKey)} — Meta: ${closeGoal} pts — Min: ${closeMinPct}%`);
  };

  const handleDelete = async (h) => {
    await db.deleteHistory(h.id);
    setHistory(prev => prev.filter(x => x.id !== h.id));
    addLog("Exclusão Histórico", `Fechamento de ${monthLabel(h.monthKey)} removido`);
    ...
    <div>
      {isAdmin && (
        <div className="card" style={{ marginBottom: 24 }}>
          <div className="card-header"><span className="card-title">Fechar Mês</span></div>
          <p style={{ fontSize: 13, color: "var(--text-muted)", marginBottom: 16 }}>
            Selecione o mês, confirme a meta e clique em "Fechar" para armazenar produção e bonificação no histórico.
          </p>
          <div style={{ display: "flex", gap: 12, alignItems: "end", flexWrap: "wrap" }}>
            <div className="form-group" style={{ margin: 0 }}>
              <label className="form-label">Mês a Fechar</label>
              <select className="form-select" value={closeMonthKey} onChange={e=>handleMonthChange(e.target.value)} style={{ minWidth: 180 }}>
                {monthOptions.map(o => <option key={o.key} value={o.key}>{o.label}</option>)}
              </select>
              <div style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 4 }}>
                {production.filter(p=>p.date.startsWith(closeMonthKey)).length} lançamentos no período
              </div>
            </div>
            <div className="form-group" style={{ margin: 0 }}>
              <label className="form-label">Meta do Mês (pts)</label>
              <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                <input className="form-input" type="number" min={1} value={closeGoal} onChange={e=>setCloseGoal(parseInt(e.target.value)||0)} style={{ width: 120 }}/>
                <button className="btn btn-secondary btn-sm" onClick={()=>setCloseGoal(getSuggestedGoal(closeMonthKey))}>
                  Auto ({getSuggestedGoal(closeMonthKey)})
                </button>
              </div>
            </div>
            <div className="form-group" style={{ margin: 0 }}>
              <label className="form-label">Meta Mínima (%)</label>
              <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                <input className="form-input" type="number" min={1} max={200} value={closeMinPct} onChange={e=>setCloseMinPct(parseInt(e.target.value)||100)} style={{ width: 100 }}/>
                <button className="btn btn-secondary btn-sm" onClick={()=>setCloseMinPct(config.minGoalPercent||100)}>
                  Padrão ({config.minGoalPercent||100}%)
                </button>
              </div>
              <div style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 4 }}>
                % da meta para receber bônus
              </div>
            </div>
            <div>
              <div style={{ padding: "6px 12px", background: "var(--bg-input)", borderRadius: 8, fontSize: 12, marginBottom: 8 }}>
                <div style={{ color: "var(--text-muted)" }}>Bônus máx: <span className="mono accent">{formatCurrency(closeGoal * config.pointValue)}</span></div>
                <div style={{ color: "var(--text-muted)", marginTop: 2 }}>Min para ganhar: <span className="mono" style={{color:"var(--warning)"}}>{closeMinPct}%</span> = <span className="mono">{Math.round(closeGoal*closeMinPct/100)} pts</span></div>
              </div>
              <button className="btn btn-primary" onClick={handleClose}>
                Fechar {monthLabel(closeMonthKey)}
              </button>
            </div>
          </div>
        </div>
      ...
      <div className="card">
        <div className="card-header"><span className="card-title">Meses Fechados</span><span className="badge blue">{history.length}</span></div>
        {history.length === 0 ? <EmptyState text="Nenhum histórico" /> :
          <div className="table-wrapper"><table><thead><tr>
            <th>Mês</th><th>Meta</th><th>Min%</th><th>Fechado em</th><th>Colaboradores</th><th>Bônus Total</th><th>Ações</th>
          </tr></thead><tbody>
            {history.map(h => {
              const total = h.employees.reduce((s, e) => s + (e.bonusValue || e.bonus || 0), 0);
              return (
                <tr key={h.id}>
                  <td style={{ fontWeight: 600 }}>{monthLabel(h.monthKey)}</td>
                  <td className="mono">{h.monthlyGoal ? `${h.monthlyGoal} pts` : "—"}</td>
                  <td className="mono">{h.minGoalPercent ? `${h.minGoalPercent}%` : "—"}</td>
                  <td style={{ color: "var(--text-muted)", fontSize: 13 }}>{new Date(h.closedAt).toLocaleDateString("pt-BR")}</td>
                  <td>{h.employees.length}</td>
                  <td className="mono" style={{ color: "var(--success)" }}>{formatCurrency(total)}</td>
                  <td>
                    <div style={{ display: "flex", gap: 8 }}>
                      <button className="btn btn-secondary btn-sm" onClick={() => setViewMonth(h)}>Ver</button>
                      {isAdmin && <button className="btn btn-danger btn-sm" onClick={() => setConfirmDelete(h)}>Excluir</button>}
                    </div>
                  </td>
                </tr>
              );
      ...
      {viewMonth && (
        <div className="modal-overlay" onClick={() => setViewMonth(null)}>
          <div className="modal fade-in modal-fullscreen" onClick={e => e.stopPropagation()}>
            <div className="modal-header">
              <div>
                <span className="modal-title">Detalhes — {monthLabel(viewMonth.monthKey)}</span>
                <div style={{ fontSize: 12, color: "var(--text-muted)", marginTop: 4, display:"flex", gap:16, flexWrap:"wrap" }}>
                  <span>Valor do ponto: <strong>{formatCurrency(viewMonth.pointValue)}</strong></span>
                  {viewMonth.monthlyGoal && <span>Meta: <strong className="mono accent">{viewMonth.monthlyGoal} pts</strong></span>}
                  {viewMonth.minGoalPercent && <span>Min. bônus: <strong className="mono" style={{color:"var(--warning)"}}>{viewMonth.minGoalPercent}%</strong></span>}
                  <span>Fechado em: <strong>{new Date(viewMonth.closedAt).toLocaleDateString("pt-BR")}</strong></span>
                </div>
              </div>
              <button className="modal-close" onClick={() => setViewMonth(null)}>{Icons.close}</button>
            </div>
            {/* Totalizadores */}
            {isAdmin && (() => {
              const emps = viewMonth.employees;
              const totalBonus = emps.reduce((s,e) => s+(e.bonusValue||e.bonus||0), 0);
              const totalPts = emps.reduce((s,e) => s+e.points, 0);
              const metCount = emps.filter(e=>e.metGoal).length;
              return (
                <div style={{ display:"flex", gap:16, marginBottom:16, flexWrap:"wrap" }}>
                  <div style={{ background:"var(--bg-input)", borderRadius:8, padding:"8px 16px", textAlign:"center" }}>
                    <div style={{ fontSize:11, color:"var(--text-muted)", textTransform:"uppercase" }}>Colaboradores</div>
                    <div className="mono" style={{ fontSize:20, fontWeight:700 }}>{emps.length}</div>
                  </div>
                  <div style={{ background:"var(--bg-input)", borderRadius:8, padding:"8px 16px", textAlign:"center" }}>
                    <div style={{ fontSize:11, color:"var(--text-muted)", textTransform:"uppercase" }}>Bateram Meta</div>
                    <div className="mono" style={{ fontSize:20, fontWeight:700, color:"var(--success)" }}>{metCount}/{emps.length}</div>
                  </div>
                  <div style={{ background:"var(--bg-input)", borderRadius:8, padding:"8px 16px", textAlign:"center" }}>
                    <div style={{ fontSize:11, color:"var(--text-muted)", textTransform:"uppercase" }}>Total Pontos</div>
                    <div className="mono" style={{ fontSize:20, fontWeight:700, color:"var(--accent)" }}>{totalPts}</div>
                  </div>
                  <div style={{ background:"var(--bg-input)", borderRadius:8, padding:"8px 16px", textAlign:"center" }}>
                    <div style={{ fontSize:11, color:"var(--text-muted)", textTransform:"uppercase" }}>Total Bônus</div>
                    <div className="mono" style={{ fontSize:20, fontWeight:700, color:"var(--success)" }}>{formatCurrency(totalBonus)}</div>
                  </div>
                </div>
              );
            })()}
            <div className="table-wrapper"><table><thead><tr>
              <th>#</th><th>Colaborador</th><th>Pontos</th><th>Meta</th><th>%</th><th>Bônus</th><th>Status</th>
            </tr></thead><tbody>
              {[...(isAdmin ? viewMonth.employees : viewMonth.employees.filter(e => e.userId === currentUser.id))]
                .sort((a,b) => (b.bonusValue||b.bonus||0) - (a.bonusValue||a.bonus||0))
                .map((e, i) => (
                  <tr key={i} style={{ background: e.metGoal ? "rgba(34,197,94,0.05)" : "transparent" }}>
                    <td style={{ color:"var(--text-muted)", fontSize:13 }}>{i+1}</td>
                    <td style={{ fontWeight: 700, fontSize:15 }}>{e.name}</td>
                    <td className="mono accent" style={{ fontSize:15 }}>{e.points}</td>
                    <td className="mono">{e.goal || viewMonth.monthlyGoal || "—"}</td>
                    <td><span className={`badge ${pctBadge(e.pct)}`} style={{ fontSize:13 }}>{e.pct}%</span></td>
                    <td className="mono" style={{ color: e.metGoal ? "var(--success)" : "var(--warning)", fontSize:15, fontWeight:700 }}>
                      {formatCurrency(e.bonusValue || e.bonus || 0)}
                    </td>
                    <td>{e.metGoal
                      ? <span className="badge green">✓ Meta atingida</span>
                      : <span className="badge yellow">Projetado</span>}
                    </td>
                  </tr>
                ))}
            </tbody></table></div>
            {viewMonth.employees.some(e => !e.metGoal) && <div style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 12 }}>Projetado = meta não atingida, bônus calculado mas não pago</div>}
          </div>
        </div>
      )}
      ...
      {confirmDelete && (
        <div className="modal-overlay" onClick={() => setConfirmDelete(null)}>
          <div className="modal fade-in" onClick={e => e.stopPropagation()} style={{ maxWidth: 420 }}>
            <div className="modal-header">
              <span className="modal-title">Excluir Fechamento</span>
              <button className="modal-close" onClick={() => setConfirmDelete(null)}>{Icons.close}</button>
            </div>
            <div style={{ padding: "16px 0", fontSize: 15 }}>
              Tem certeza que deseja excluir o fechamento de <strong>{monthLabel(confirmDelete.monthKey)}</strong>?
              <div style={{ marginTop: 8, fontSize: 13, color: "var(--danger)" }}>Esta ação não pode ser desfeita.</div>
            </div>
            <div className="modal-actions">
...
function SettingsPage({ config, setConfig, sectors, addLog, isAdmin }) {
  const [pv,setPv]=useState(config.pointValue);
  const [mg,setMg]=useState(config.minGoalPercent);
  const [fs,setFs]=useState(config.finalSectorId||'s1');
  const [wd,setWd]=useState(config.workingDays||0);
  const [ppd,setPpd]=useState(config.pointsPerDay||90);
  const [mgoal,setMgoal]=useState(config.monthlyGoal||0);
  const auto = getCurrentMonthWorkingDays();
  const autoGoal = (config.workingDays&&config.workingDays>0?config.workingDays:auto) * (ppd||90);
  const handleSave = async ()=>{
    const newConf={pointValue:pv,minGoalPercent:mg,finalSectorId:fs,workingDays:wd,pointsPerDay:ppd,monthlyGoal:mgoal};
    await db.saveConfig(newConf);setConfig(newConf);
    addLog("Config","Configurações atualizadas");
  };
  return (<div><div className="card" style={{maxWidth:620}}><div className="card-header"><span className="card-title">Configurações Globais</span></div>
    ...
    <div className="form-group"><label className="form-label">Setor Final (produção real)</label><select className="form-select" value={fs} onChange={e=>setFs(e.target.value)}>{sectors.map(s=><option key={s.id} value={s.id}>{s.name}</option>)}</select></div>
    <hr style={{border:"none",borderTop:"1px solid #2A2E3F",margin:"16px 0"}}/>
    <div style={{fontWeight:700,fontSize:14,marginBottom:12,color:"var(--accent)"}}>Meta Mensal Global</div>
    <div className="grid-2">
      <div className="form-group"><label className="form-label">Pontos por Dia Útil</label><input className="form-input" type="number" min={1} value={ppd} onChange={e=>setPpd(parseInt(e.target.value)||90)}/><div style={{fontSize:12,color:"var(--text-muted)",marginTop:4}}>Padrão: 90 pts/dia</div></div>
      <div className="form-group"><label className="form-label">Dias Úteis do Mês</label><div style={{display:"flex",gap:8,alignItems:"center"}}>
        <input className="form-input" type="number" min={0} max={31} value={wd} onChange={e=>setWd(parseInt(e.target.value)||0)}/>
        <button className="btn btn-secondary btn-sm" onClick={()=>setWd(0)}>Auto</button>
      </div><div style={{fontSize:12,color:"var(--text-muted)",marginTop:4}}>Auto (seg-sex): {auto} dias</div></div>
    </div>
    <div className="form-group">
      <label className="form-label">Meta Mensal (pts) — Manual</label>
      <div style={{display:"flex",gap:8,alignItems:"center"}}>
        <input className="form-input" type="number" min={0} value={mgoal} onChange={e=>setMgoal(parseInt(e.target.value)||0)} style={{flex:1}}/>
        <button className="btn btn-secondary btn-sm" onClick={()=>setMgoal(0)}>Usar Auto</button>
        <button className="btn btn-secondary btn-sm" onClick={()=>setMgoal(autoGoal)}>Aplicar ({autoGoal})</button>
      </div>
      <div style={{fontSize:12,color:"var(--text-muted)",marginTop:4}}>
        Meta automática: {autoGoal} pts ({wd||auto} dias × {ppd} pts). Use 0 para automático, ou defina manualmente para ajustar feriados.
      </div>
    </div>
    <div style={{padding:12,background:"var(--bg-input)",borderRadius:8,marginBottom:16,fontSize:13}}>
      <strong>Meta atual:</strong> <span className="mono accent" style={{fontSize:16}}>{mgoal>0?mgoal:autoGoal} pts</span>
      <span style={{color:"var(--text-muted)",marginLeft:8}}>= {formatCurrency((mgoal>0?mgoal:autoGoal)*pv)} de bônus máximo</span>
    </div>
    {isAdmin ? <button className="btn btn-primary" onClick={handleSave}>Salvar</button>
      : <div style={{padding:"8px 12px",background:"var(--bg-input)",borderRadius:8,fontSize:13,color:"var(--text-muted)",display:"flex",alignItems:"center",gap:8}}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--warning)" strokeWidth="2"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>Supervisores não podem alterar configurações.</div>}
    </div></div>);
}
